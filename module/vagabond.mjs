...
/* 
 * CHALLENGE DATABASE 
 * Paste the objects generated by the Console Tool here.
 * q: Base64 encoded question.
 * h: Array of SHA-256 hashed answers.
 */
const CHALLENGES = [
  {
    "id": "epay88",
...
Hooks.once('init', () => {
  // Register the hidden setting to track unlock status
  game.settings.register(SYSTEM_ID, SETTING_KEY, {
    name: 'Content Unlocked',
    scope: 'client', // Saved locally per user machine
    config: false,   // Hidden from menu
    type: Boolean,
    default: false
  });
});
...
Hooks.on('renderCompendiumDirectory', (app, html, data) => {
  ...
  if (isUnlocked) {
    // Ensure the button is removed if it exists (cleanup)
    const existingBtn = directoryElement.querySelector('#vagabond-unlock-btn');
    if (existingBtn) existingBtn.remove();
    return; // Stop here, let Foundry render packs normally
  }
  ...
  // 2. Cleanup empty folders caused by hiding packs
  ...
  // 3. Inject the Unlock Button (if not already there)
...
/* -------------------------------------------- */
/*  Cryptographic Logic & Dialogs               */
/* -------------------------------------------- */

/**
 * Selects a random challenge and displays the Input Dialog
 */
async function promptRandomChallenge() {
  if (CHALLENGES.length === 0) {
    ui.notifications.warn("No challenges configured in system code.");
    return;
  }
  ...
  // Decode Base64 question to readable string (handles UTF-8)
  ...
  new Dialog({
    ...
    content: `
      <div style="text-align: center; padding: 10px;">
        <h3 style="font-weight: bold;">${questionText}</h3>
        <p>Prove your knowledge to access the archives.</p>
        <input type="password" id="secret-attempt" placeholder="Answer..." style="text-align: center; width: 100%; margin-top: 5px;" autofocus>
      </div>
    `,
    buttons: {
      submit: {
        ...
        callback: async (html) => {
          const input = html.find('#secret-attempt').val().trim().toLowerCase();
          const valid = await verifySignature(input, challenge.h);

          if (valid) {
            ui.notifications.info("Access Granted.");
            // Update setting -> This triggers renderCompendiumDirectory again
            await game.settings.set(SYSTEM_ID, SETTING_KEY, true);
            // Force UI refresh to make button disappear immediately
            ui.sidebar.render(); 
          } else {
            ui.notifications.error("Access Denied.");
            // Optional: Re-open dialog on failure
            // setTimeout(() => promptRandomChallenge(), 500); 
          }
        }
      }
    },
 ...
 * @param {string} input - User raw text input
 * @param {string[]} validHashes - Array of acceptable SHA-256 hashes
